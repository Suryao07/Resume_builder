...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const downloadResume = async () => {
        try {
            ...
            // Add watermark (hidden)
            doc.setFontSize(6);
            doc.setTextColor(255, 255, 255, 0.01); // Almost invisible
            doc.text('Created with Resume Builder', pageWidth/2, pageHeight - 5, { align: 'center' });
            
            // Prepare a clone for conversion
            const clone = resumeElement.cloneNode(true);
            
            // Apply improved styling to clone
            ...
            const improveTextStyle = (elements, property, value) => {
                elements.forEach(el => {
                    if (el) el.style[property] = value;
                });
            };
            
            // Apply consistent styling to text elements
            const headers = clone.querySelectorAll('h1, h2, h3, h4, h5, h6');
            const paragraphs = clone.querySelectorAll('p, span, div');
            const lists = clone.querySelectorAll('ul, ol, li');
            
            improveTextStyle(headers, 'letterSpacing', '0.02em');
            improveTextStyle(paragraphs, 'letterSpacing', '0.02em');
            improveTextStyle(lists, 'letterSpacing', '0.02em');
            
            improveTextStyle(headers.concat(Array.from(paragraphs)).concat(Array.from(lists)), 'lineHeight', '1.4');
            
            improveTextStyle(headers.concat(Array.from(paragraphs)).concat(Array.from(lists)), 'wordSpacing', '0.05em');
            
            // Skills need special handling
            const skills = clone.querySelectorAll('#skills-container span');
            skills.forEach(skill => {
                skill.style.display = 'inline-block';
                skill.style.margin = '0.2em';
            });
            
            // Create a temporary container and add the clone
            ...
            // Advanced method for PDF generation to avoid duplication
            try {
                ...
                const canvas = await html2canvas(clone, {
                    ...
                    onclone: (clonedDoc) => {
                        ...
                        style.innerHTML = `
                            p, div, span { letter-spacing: 0.02em !important; word-spacing: 0.05em !important; line-height: 1.4 !important; }
                            h1, h2, h3, h4, h5, h6 { letter-spacing: 0.02em !important; line-height: 1.3 !important; }
                        `;
                ...
                const onePageHeight = (pageHeight - 20) / ratio; // 10mm margin top and bottom
                const totalPages = Math.ceil(imgHeight / onePageHeight);
                ...
                for (let i = 0; i < totalPages; i++) {
                    ...
                    // Calculate the source area to capture for this page
                    const sourceY = i * onePageHeight;
                    const remainingHeight = Math.min(onePageHeight, (imgHeight - sourceY));
                    
                    // Add the image to the current page, clipped to the current section
                    doc.addImage(
                        imgData,
                        'JPEG', 
                        0, 10, // 10mm margin from top
                        pageWidth, 
